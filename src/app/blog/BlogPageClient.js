'use client';

import { useSearchParams } from 'next/navigation';
import Link from 'next/link';
import styles from './Blog.module.css';


const POSTS_PER_PAGE = 6;

export default function BlogPageClient({ posts }) {
  const searchParams = useSearchParams();
  const currentPage = parseInt(searchParams.get('page') || '1', 10);
  const totalPages = Math.ceil(posts.length / POSTS_PER_PAGE);
  const startIndex = (currentPage - 1) * POSTS_PER_PAGE;
  const paginatedPosts = posts.slice(startIndex, startIndex + POSTS_PER_PAGE);

  return (
    <main className={styles.blogWrapper}>
      <section className={styles.hero}>
        <h1>Smart Home Blog</h1>
        <p>Tips, guides, and ideas to level up your smart home. Learn to automate better, save energy, and take full control.</p>
      </section>

      <div className={styles.postGrid}>
        {paginatedPosts.map((post) => (
          <div key={post.slug} className={styles.postCard}>
            <h3>{post.title || 'Untitled'}</h3>
            <p className={styles.meta}>{new Date(post.date).toLocaleDateString('en-GB')}</p>
            <p>{post.description || 'No summary available.'}</p>
            <Link href={`/blog/${post.slug}`} className={styles.ctaSmall}>Read More →</Link>
          </div>
        ))}
      </div>

      {totalPages > 1 && (
        <div className={styles.pagination}>
          {currentPage > 1 && (
            <Link href={`?page=${currentPage - 1}`} className={styles.pageButton}>
              ← Previous
            </Link>
          )}
          {Array.from({ length: totalPages }, (_, i) => (
            <Link
              key={i}
              href={`?page=${i + 1}`}
              className={`${styles.pageButton} ${currentPage === i + 1 ? styles.pageButtonActive : ''}`}
            >
              {i + 1}
            </Link>
          ))}
          {currentPage < totalPages && (
            <Link href={`?page=${currentPage + 1}`} className={styles.pageButton}>
              Next →
            </Link>
          )}
        </div>
      )}
    </main>
  );
}
